...
def _cvt_equal_mass(x, y, signal, noise, xnode, ynode, pixelsize, quiet, sn_func, wvt):
    ...
    for it in range(min([xnode.size, 100])):  # Do at most xnode.size iterations
    # FB changed this on 02 Jan 2020 to avoid getting stuck in this loop
    # for it in range(xnode.size):  # Do at most xnode.size iterations
        ...
        good = np.unique(classe)
        if wvt:
            # OE: these lines below were change to fix a bug (different shapes of the xnode and xnode_old)
            for k in good:
